    ...
    public function getMethod($address, $total)
    {
        ...
        $query = $this->db->query(
            "SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get(
                'payment_payatrader_geo_zone_id'
        ...
        if ($status) {
            if (array_key_exists(
                    'currency',
                    $this->cart->customer->request->request
                ) && $this->cart->customer->request->request['currency'] != 'GBP' && !$this->config->get(
                    'payment_payatrader_forceshow'
                )) {
                $status = false;
            }
        }
